...
<div align="center">
 ...
 <details> 
  ...
  - **Multi-monitor wallpaper support**: Per-monitor wallpaper management with automatic detection
  - **Transparent installation**: Every command is shown before it's run
</details>
<details>
  <summary>Multi-Monitor Wallpaper Setup</summary>

  The quickshell configuration includes advanced multi-monitor wallpaper management:

  ### Features:
  - **Automatic monitor detection** via `hyprctl`
  - **Per-monitor wallpaper storage** in config (`background.wallpaperPaths`)
  - **Migration from single-monitor** configurations
  - **Apply to all monitors** or specific monitors

  ### Usage:
  1. Open wallpaper selector (default: likely `Super+W` or check keybind list)
  2. Browse to wallpaper folder
  3. Use monitor dropdown to select:
     - **"Apply to all monitors"**: Same wallpaper on all displays
     - **Specific monitor name**: Apply only to that display
  4. Click wallpaper thumbnail to apply

  ### Configuration:
  - Stored in `~/.config/illogical-impulse/config.json`
  - Structure: `background.wallpaperPaths["DP-1"] = "/path/to/wallpaper.jpg"`
  - Backward compatible with single `wallpaperPath`

  ### Script Integration:
  The `switchwall.sh` script supports:
  - `--monitor <name>`: Apply to specific monitor
  - `--all-monitors`: Apply to all monitors
